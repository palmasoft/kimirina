...
class InformeMensualIntervencionPromotor {
    ...
    static function generar_periodo( $subreceptor, $datosInforme = null, $periodo = '', $nombreProvincia = '', $nombreCanton = '', $nombrePromotor = '', $nombreGenerador = '', $tiposPobSubreceptor ) {
        $ruta = 'archivos' . DS . 'informes' . DS . $subreceptor->SIGLAS_SUBRECEPTOR . DS . 'promotores' . DS . 'intervencion' . DS;
        $nombre = 'informe_mensual_intervencion_promotores_' . uniqid() . '.pdf';
        Archivos::probar_crear_directorio($ruta);
        self::generar(
                'Informe intervencion mensual promotores.',
                'INTERVENCION MENSUAL PROMOTORES',
    ...
    static function generar_trimestre( $subreceptor, $datosInforme = null, $periodo = '', $nombreProvincia = '', $nombreCanton = '', $nombrePromotor = '', $nombreGenerador = '', $tiposPobSubreceptor ) {
        $ruta = 'archivos' . DS . 'informes' . DS . $subreceptor->SIGLAS_SUBRECEPTOR . DS . 'promotores' . DS . 'intervencion' . DS;
        $nombre = 'informe_trimestral_intervencion_promotores_' . uniqid() . '.pdf';
        Archivos::probar_crear_directorio($ruta);
        ...
        self::generar(
                'Informe intervencion trimestral promotores.',
                'INTERVENCION TRIMESTRAL PROMOTORES',
                $CODIGOS,  $ruta, $nombre,
                $subreceptor, $datosInforme, $periodo, $nombreProvincia, $nombreCanton, $nombrePromotor, $nombreGenerador, $tiposPobSubreceptor);
    ...
    public static function generar($titulo, $titulo_informe, $CODIGOS, $ruta, $nombre, $subreceptor, $datosInforme = null, $periodo = '', $nombreProvincia = '', $nombreCanton = '', $nombrePromotor = '', $nombreGenerador = '', $tiposPobSubreceptor) {
        ...
        self::$generador->Cell(0, 7, 'ANEXO 7.6.1', 0, true, 'C', 0, '', 0, false, 'M', 'M');
...
//        
    ...
    static function generar_periodo_xls( $subreceptor, $datosInforme = null, $periodo = '', $nombreProvincia = '', $nombreCanton = '', $nombrePromotor = '', $nombreGenerador = '', $tiposPobSubreceptor ) {
        $ruta = 'archivos' . DS . 'informes' . DS . $subreceptor->SIGLAS_SUBRECEPTOR . DS . 'promotores' . DS . 'intervencion' . DS;
        $nombre = 'informe_mensual_intervencion_promotores_' . uniqid() . '.xls';
        Archivos::probar_crear_directorio($ruta);
    ...
    static function generar_trimestre_xls( $subreceptor, $datosInforme = null, $periodo = '', $nombreProvincia = '', $nombreCanton = '', $nombrePromotor = '', $nombreGenerador = '', $tiposPobSubreceptor ) {
        $ruta = 'archivos' . DS . 'informes' . DS . $subreceptor->SIGLAS_SUBRECEPTOR . DS . 'promotores' . DS . 'intervencion' . DS;
        $nombre = 'informe_trimestral_intervencion_promotores_' . uniqid() . '.xls';
        Archivos::probar_crear_directorio($ruta);
    ...
    public static function generar_xls($titulo, $CODIGOS, $ruta, $nombre, $subreceptor, $datosInforme = null, $periodo = '', $nombreProvincia = '', $nombreCanton = '', $nombrePromotor = '', $nombreGenerador = '', $tiposPobSubreceptor){
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('E'.$fila.'', '')
                ->setCellValue('F'.$fila.'', '' )
                ->setCellValue('G'.$fila.'', 'ANEXO 7.6.1' )
                ->setCellValue('H'.$fila.'', '' )
                ->setCellValue('I'.$fila.'', '' )
                ->setCellValue('J'.$fila.'', '')
                ->setCellValue('K'.$fila.'', '')
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A6', 'Espacios De Socializacion Y Trabajo Sexual')
                    ->setCellValue('B6', 'Frecuencia(No.Deveces)De Espacios Visitados')
                    ->setCellValue('C6', 'TS')
        ...
        foreach ($datosInforme->filas as $informe) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A'.$fila.'', $informe->NOMBRE_TIPOLUGAR)
                    ->setCellValue('B'.$fila.'', $informe->FRECUENCIA )
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C'.$fila.'', $datosInforme->totalTS)
                    ->setCellValue('D'.$fila.'', $datosInforme->totalTRANS)
                    ->setCellValue('E'.$fila.'', $datosInforme->totalHSH)
                    ->setCellValue('F'.$fila.'', $datosInforme->totalTS+$datosInforme->totalHSH+$datosInforme->totalTRANS);

        $objPHPExcel->getActiveSheet()->setTitle('Intervenciones del Promotor ' . $nombrePromotor );
        $objPHPExcel->setActiveSheetIndex(0);
    ...
    public static function producir_html($datosInforme = null, $periodo = '', $nombrePromotor = '', $nombreGenerador = '', $tiposPobSubreceptor) {
        ...
        $html .= '
            <tr>
                <td rowspan="2">Espacios De Socializacion Y Trabajo Sexual</td>
                <td rowspan="2">Frecuencia(No.Deveces)De Espacios Visitados</td>
                <td colspan="4"></td>
            </tr>
            <tr>
                <td>TS</td>
        ...
        $html .= '
            <tr>
                ...
                <td >'.$datosInforme->totalTS.'</td>               
                <td >'.$datosInforme->totalTRANS.'</td>
                 <td >'.$datosInforme->totalHSH.'</td>
                <td >'.($datosInforme->totalTS+$datosInforme->totalHSH+$datosInforme->totalTRANS).'</td>
            </tr> ';
        $html .= '</table>';
