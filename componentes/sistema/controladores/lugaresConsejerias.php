...
class lugaresConsejeriasControlador extends ControllerBase {
    ...
    function guardar_nuevo_lugar_consejeria() {
        $idlugarconsejeria = LugaresConsejeriaModel::insertar(
                        $this->datos['codigo_lugar_consejeria'], $this->datos['nombre_lugar_consejeria'], $this->datos['observaciones_lugar_consejeria']
        );

        if ($idlugarconsejeria > 0) {
            echo '{ "resultado":"EXITO", "mensaje":"Lugar de Consejeria ' . $this->datos['nombre_lugar_consejeria'] . ' Guardado Exitosamente"}';
        } else {
    ...
    function editar_consejeria() {
        $idlugarconsejeria = LugaresConsejeriaModel::update(
                        $this->datos['registro-id'], $this->datos['codigo_lugar_consejeria'], $this->datos['nombre_lugar_consejeria'], $this->datos['observaciones_lugar_consejeria']
        );
        if ($idlugarconsejeria > 0) {
            echo '{"resultado":"EXITO", "mensaje":"Lugar de Consejeria ' . $this->datos['nombre_lugar_consejeria'] . ' Modificado Exitosamente"}';
        } else {
            echo '{"resultado":"ERROR", "mensaje":"No se ha podido guardar. intentelo nuevamente y si elproblema persite consulte al administrador del sistema."}';
        }
    }
