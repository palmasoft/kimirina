...
//        $this->datos['centrosSalud'] = CentrosSaludModel::por_subreceptor($_SESSION['SESION_USUARIO']->ID_SUBRECEPTOR);
    ...
    function mostrar_datos_registro_atencion_modal() {
        $this->datos_para_formulario();
        $this->vista->mostrar('registro_atencion_salud/datosRegistroAtencionSalud', $this->datos);
    }
    ...
    function guardar_nuevo_registro_atencion_salud() {
        ...
        if (!$idPemar) {
            $idPemar = PemarsModel::insertar_sin_sexo(
                    $this->datos['tiposPemars'], $this->datos['codigo-pemar'], 
                    $this->datos['mes-nacimiento'], $this->datos['ano-nacimiento'], 
                    $this->datos['nombreUno'], $this->datos['nombreDos'], 
                    $this->datos['apellidoUno'], $this->datos['apellidoDos'], $this->datos['cedula'], ""
            );
        }
        ...
        $idRegistroSalud = registroAtencionSaludModel::insertar(
                        $this->datos['centroSalud'], TiposPoblacionModel::datos($this->datos['tiposPemars'])->CODIGO_TIPOPOBLACION, $idPemar, $this->datos['fechaAtencion'], $this->datos['horaAtencion'], $this->datos['tipoServicio'], $this->datos['subreceptor-atencion'], $this->datos['nombreUno'], $this->datos['nombreDos'], $this->datos['apellidoUno'], $this->datos['apellidoDos'], $this->datos['cedula'], $verificado
        );

        if ($idRegistroSalud > 0) {
            echo '{ "resultado":"EXITO", "mensaje":" Registro de <strong>Atención en Unidad de Salud</strong> Guardado Exitosamente."}';
        } else {
    ...
    function actualizar_datos_atencion_salud() {
        ...
        $idRegistroSalud = registroAtencionSaludModel::update(
                        ...
                        $this->datos['fechaAtencion'], $this->datos['horaAtencion'], $this->datos['tipoServicio'], $this->datos['subreceptor-atencion'], $this->datos['nombreUno'], $this->datos['nombreDos'], $this->datos['apellidoUno'], $this->datos['apellidoDos'], $this->datos['cedula'], $verificado
        );
    ...
    function editar_registro_atencion_salud() {
        ...
        if ($idRegistroSalud > 0) {
            echo '{"resultado":"EXITO", "mensaje":"Registro de <strong>Atención en Unidad de Salud</strong> Modificado Exitosamente"}';
        } else {
