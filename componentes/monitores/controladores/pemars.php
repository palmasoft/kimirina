...
class pemarsControlador  extends ControllerBase {
    ...
    function guardar_nueva_pemar(){
        ...
        if( $idpemar > 0 ){
            echo '{ "resultado":"EXITO", "mensaje":"Se registrado El nuevo PEMAR '.$this->datos['nombre_uno_poblacion'].' '.$this->datos['nombre_dos_poblacion'].' '.$this->datos['apellido_uno_poblacion'].' '.$this->datos['apellido_dos_poblacion'].' Exitosamente"}';
        }else{
            echo '{"resultado":"ERROR", "mensaje":"No se ha podido guardar. Asegurece que el codigo generado no exista antes de guardarlo. intentelo nuevamente y si elproblema persite consulte al administrador del sistema."}';
        }
    }
