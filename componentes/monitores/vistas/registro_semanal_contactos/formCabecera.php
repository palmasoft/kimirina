<form id="form-registro-semanal-contactos" method="post" class="form-inline" onsubmit="return false;">  
    ...
    <div style="text-align: center;">
        <?php echo ($this->datos['SubReceptor']->ID_SUBRECEPTOR == 0) ? "<h5>SubReceptor: <strong>".(isset($datosRegistroSemanal) ? $datosRegistroSemanal->SIGLAS_SUBRECEPTOR : "DEPENDE DEL PROMOTOR" )."</strong></h5>" : "" ?>
        <h5>PERIODO / MES: <input type="text" id="periodo-contactos" name="periodo-contactos" class="input-small"
                                  value="<?php echo isset($datosRegistroSemanal) ? $datosRegistroSemanal->PERIODO_REGISTROSEMANAL : $periodoActual->CODIGO_PERIODO ?>" readonly="" /></h5>
    </div>
...
<script >
    ...
    $(document).ready(function() {
        ...
        $('.btn_limpiar_registro_semanal_contactos').on('click', function(evt, params) {
           confirm('¿Seguro que desea Liimpiar o borrar los datos digitados de la <strong>Hoja de Registro Semanal de Alcances</strong> ?','limpiar_formulario_encabezado()');
        });
    });
    
    function limpiar_formulario_encabezado(){
        
        $('#provincia-chosen option[value=""]').attr("selected", "selected");        
        $('#provincia-chosen').change();
        $('#provincia-chosen').trigger("liszt:updated");
        
        $('#sel-lista-cantones option[value=""]').attr("selected", "selected");        
        $('#sel-lista-cantones').trigger("liszt:updated");
        
        $('#promotor-formulario option[value=""]').attr("selected", "selected");        
        $('#promotor-formulario').trigger("liszt:updated");
