<form id="form-registro-atencion" class="form-inline" onsubmit="return false;" >  
    ...
    <div class="block block-themed ">
        <div class="block-title"><h4>
                ...
                Datos de la Atención en Unidad de Salud</h4></div>
        <div class="block-content">
            ...
            <!-- TIPO DE SERVICIO -->
            <div class="control-group form-horizontal">
                <label class="control-label" for="horizontal-select">Tipo de Atención o Servicio:</label>
                <div class="controls"> 
            ...
            <div class="control-group form-horizontal">
                ...
                <div class="controls">
                    ...
                    <span class="help-block" id="img_consulta_registraduria" style="display: none;" >                        
                        <img src="imagenes/cargando_horizontal.gif" style="height: 32px;" /> Estamos consultando el sitio web de la registraduría por usted. Por favor, espere...
                    </span>
            ...
            <div class="control-group form-horizontal">
                ...
                <div class="controls">
                    <input type="text" id="primer-nombre" name="nombreUno" class="soloLetras  validar_cedula_codigo span3  mayusculas " 
                           value="<?php echo isset($registrosAtencion) ? ($registrosAtencion->PRIMER_NOMBRE_PEMAR) : ''; ?>" placeholder="Primer nombre"  />
                    <input type="text" id="segundo-nombre" name="nombreDos" class="soloLetras  validar_cedula_codigo span3 mayusculas " 
                           value="<?php echo isset($registrosAtencion) ? ($registrosAtencion->SEGUNDO_NOMBRE_PEMAR) : ''; ?>" placeholder="Segundo nombre"  />
                    <input type="text" id="primer-apellido" name="apellidoUno" class="soloLetras  validar_cedula_codigo span3  mayusculas" 
                           value="<?php echo isset($registrosAtencion) ? ($registrosAtencion->PRIMER_APELLIDO_PEMAR) : ''; ?>" placeholder="Primer Apellido"  />
                    <input type="text" id="segundo-apellido" name="apellidoDos" class="soloLetras validar_cedula_codigo span3  mayusculas" 
            ...
            <div class="control-group form-horizontal">
                ...
                <div class="controls "> 
                    ...
                    $this->formularios->lista_mes('nacimiento', ' cambiar_tamaño1 validar_cedula_codigo ', $valor);
                    ?>
                    ...
                    $this->formularios->lista_ano('nacimiento', ' cambiar_tamaño1 validar_cedula_codigo ', $valor);
                    ?>
                </div>
            </div>
            ...
            <div class="control-group form-horizontal">
                ...
                <div class="controls">
                    <select id="subreceptores" name="subreceptor-atencion" class="select-chosen " >                        
                        <?php
    ...
    <div class="block block-themed block-last">
        <div class="form-actions" style="text-align: center;">
            ...
            <button id="btn_limpiar_atencion_salud" type="button" class="btn btn-danger"><i class="icon-repeat"></i> Limpiar</button>
            <button type="button" id="btn_guardar_atencion_salud" class="btn btn-success" disabled="" ><i class="icon-save"></i> Guardar</button>
        </div>
    </div>
...
<div id="noEstaCedulaRegistraduria" style="display: none" >
    <?php echo AlertasHTML5::informacion('La cédula no se encuentra en la lista de la registraduría o el sitio web esta caido. Verifica que el sitio <a href="http://www.corporacionregistrocivil.gov.ec/" target="_blank" >http://www.corporacionregistrocivil.gov.ec/</a> este habilitado.'); ?>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('#btn_guardar_atencion_salud').on('click', function(evt, params) {
            $('#form-registro-atencion').submit();
        });
        $('#btn_limpiar_atencion_salud').on('click', function(evt, params) {
            confirm(
                '¿Seguro que desea limpiar el formulario o <strong>borrar los datos digitados</strong> de la atención en salud?'
                , 'resetear_formulario();'
            );
        });
    ...
    function resetear_formulario() {
        ...
        //$('#cedula-atendido').attr('value','');
        $('#codigo-pemar-generado').attr('value','');

        $('#chk-cedula-verificada').iCheck('uncheck');
        
        $('#tipoServicio option[value=""]').attr("selected", "selected");  
        $('#tipoServicio').trigger("liszt:updated");
                
        $('#centroSalud option[value=""]').attr("selected", "selected");            
        $('#centroSalud').trigger("liszt:updated");
        
        $('#tiposPemars option[value=""]').attr("selected", "selected");  
        $('#tiposPemars').trigger("liszt:updated");
                
        document.getElementById('form-registro-atencion').reset();
    }
    ...
    function generarCodigo() {
        ...
        <?php if (Usuario::esDNI()): ?>
            codP = generarCodigoUnicoPemarDNI(
                    $('#primer-nombre').val(), $('#segundo-nombre').val(),
                    $('#primer-apellido').val(), $('#segundo-apellido').val(),
                    $('#mes-nacimiento').val(), $('#ano-nacimiento').val()
                    );
        <?php else: ?>
            codP = generarCodigoUnicoPemar(
                    $('#primer-nombre').val(), $('#segundo-nombre').val(),
                    $('#primer-apellido').val(), $('#segundo-apellido').val(),
                    $('#mes-nacimiento').val(), $('#ano-nacimiento').val()
                    );
    ...
    function validarDatosAtencionSalud() {
        ...
        if (estaVacio($('#centroSalud').val())) {
            alert('Seleccione el Centro donde recibe la Atención.');
            return false;
        }

        if (estaVacio($('#tiposPemars').val())) {
            alert('Seleccion el Tipo de Población del Atendido');
            return false;
        }
        ...
        if (estaVacio($('#subreceptores').val())) {
            alert('Seleccione el Subreceptor relacionado a la Atención.');
            return false;
        }
