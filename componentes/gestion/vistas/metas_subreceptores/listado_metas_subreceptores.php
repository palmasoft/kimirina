...
<div id="page-content">
    ...
    <div class="row-fluid" >
        <div class="span3 text-right" >Modificar Esquema de Metas</div>
                ...
                foreach ($subreceptores as $subreceptor) {
                    ...
                    if (isset($datosSubreceptores))
                        if ($subreceptor->ID_SUBRECEPTOR == $datosSubreceptores->ID_SUBRECEPTOR)
                            $selected = " selected ";
                    ?>
                    <option value="<?php echo $subreceptor->ID_SUBRECEPTOR ?>"   <?php echo $selected; ?> ><?php echo ($subreceptor->SIGLAS_SUBRECEPTOR) ?>- <?php echo ($subreceptor->NOMBRE_SUBRECEPTOR) ?></option>
                <?php } ?>
            </select>            
        </div>
        <div class="span3 text-left " ><a href="javascript:abrir_form_nuevas_metas_subreceptor();" data-toggle="tooltip" title="Actualizar Metas" class="btn btn-lg btn-success"><i class="icon-pencil"></i> Actualizar Metas </a></div>
    </div>
    <hr />
    ...
    <div class=" row-fluid botones_arriba" >
        <div class=" span2 btn-group text-left">	
            <a style="display:none;" href="javascript:abrir_form_nueva_meta_subreceptor();" data-toggle="tooltip" title="Agregar nuevo Formulario al Registro" class="btn btn-lg btn-info"><i class="icon-plus"></i> Nueva Meta</a>
        </div>
        <div class="span6 text-center">
...
<script>
    agregar_boton_ayuda('LISTAMETASSUBRECEPTOR');
</script>