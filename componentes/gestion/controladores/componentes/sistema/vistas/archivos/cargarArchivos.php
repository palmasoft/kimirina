...
<form id="form-registro-semanal-soportes" onsubmit="return false;"><div class="row-fluid">
    <?php if (!empty($datosRegistroSemanal->SOPORTES)) {
            ...
            <div class="span3" id="soporte_<?php echo intval($archivos->ID_SOPORTE_REGISTROSEMANAL) ?>">
                <div class="block block-themed themed-default">
                    <div class="block-title">
                        <h4><a href="javascript:void(0);" class="btn btn-danger enable-tooltip" title="Eliminar Soporte" data-original-title="Eliminar" onclick="confirm('¿seguro que desea eliminar el archivo?', '$(\'#soporte_<?php echo
        intval($archivos->ID_SOPORTE_REGISTROSEMANAL) ?>\').remove();');" ><i class="icon-trash"></i></a> Archivo <?php echo
            intval($archivos->ID_SOPORTE_REGISTROSEMANAL) ?>: <?php echo $archivos->TIPO_SOPORTE_REGISTROSEMANAL ?></h4>
                    </div>
                    <div class="block-content full">
                        <a href="javascript:void(0);" onclick="abrir_soportes('archivos/<?php echo $archivos->
            RUTA_SOPORTE_REGISTROSEMANAL ?>', 'Soporte del Registro <?php echo ($datosRegistroSemanal->
            NUM_REGISTROSEMANAL); ?>.');"  >
                            <img src="imagenes/archivos/<?php echo $archivos->TIPO_SOPORTE_REGISTROSEMANAL; ?>.png" />
                        </a>
                    </div>
                    <input type="hidden" name="archivo-asociado[]" value="<?php echo $archivos->
        ID_SOPORTE_REGISTROSEMANAL ?>" />
                </div>
            </div>
            <?php endforeach;
} ?>
